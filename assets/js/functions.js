...
function highlightText(element, keyword) {
  ...
  const newText = text.replace(new RegExp(keyword, 'gi'), match => `<span class="highlight">${match}</span>`);
...
function copyToClipboard() {
  try {
      ...
      var referralLink = document.getElementById("referralLink").textContent.trim();

      // Basic validation: Check if the text is a valid URL
      if (!isValidURL(referralLink)) {
          alert('Invalid referral link. Please contact support.');
          return;
      }
      ...
      navigator.clipboard.writeText(referralLink).then(function() {
          ...
          // Optionally, change back to original after a short delay
          setTimeout(function() {
